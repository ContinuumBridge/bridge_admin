...
def shc_email(user, password, start, bid, to, key):
    ...
    col = 1
    for s in serieslist:
        if not("battery" in s or "connected" in s or "luminance" in s or ("binary" and "kettle" in s)):
            ...
            for value in (ss[1], ss[2], ss[3]):
                ...
                if working == "h1":
                    ...
                    h1 = h2.replace(holder, value)
                    working = "h1"
            if series and ("door" in s.lower() or "cupboard" in s.lower()) and "binary" in s.lower():
    ...
    fp = open('image002.gif', 'rb')
    msgImage = MIMEImage(fp.read())
    fp.close()
    # Define the image's ID as referenced above
    msgImage.add_header('Content-ID', '<image002.gif>')
    msg.attach(msgImage)
